...
for stat in ["precision"]:
    ...
    for sel_dataset in tqdm(datasets, desc="Datasets"):
        ...
        for sel_model in models:
            ...
            fi = pd.read_csv(f"../results_new/{sel_dataset}/{sel_model}/fi_test.csv")
            fi_attr = fi[fi["importance"] > 0].sort_values(by="importance", ascending=False)["attribute"].values

            file_path= f"../results_all/{sel_dataset}/{sel_model}/ruleset_filterFF_{stat}_global.json"
            ...
            if not os.path.exists(file_path):            
                ...
                if len(fi_attr) > 0:
                    model = generator.fit(x_train_df, y_train, attributes_list=[fi_attr])
                else:
